...
// Preserve the availability API exposed by earlier versions of this fork.
exports.isAvailable = function (callback, scope)
{
    this.canPrintItem(callback, scope);
};
...
exports._mergeWithDefaults = function (options)
{
    ...
    if (options.orientation === undefined && options.landscape !== undefined)
        options.orientation = options.landscape ? 'landscape' : 'portrait';

    if (options.monochrome === undefined && options.graystyle !== undefined)
        options.monochrome = options.graystyle;

    if (options.printer === undefined && options.printerId !== undefined)
        options.printer = options.printerId;

    if (options.ui === undefined && Array.isArray(options.bounds))
        options.ui = { left: options.bounds[0], top: options.bounds[1],
                       width: options.bounds[2], height: options.bounds[3] };
